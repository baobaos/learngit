...
$('.form_date').datetimepicker({
	...
	var userId=result.userId;//1块查询条件
		...
		//1、循环输出列表
	if (result.allAbilityList != null && result.allAbilityList.length != 0)
	{
		for (var i = 0; i < result.allAbilityList.length; i++) 
		{
			if(result.pageNowOne==1){
					if(result.allAbilityList[i].rownum <= 3){
						$("#tbody_id").append('<tr class="gradeX">'+
						'<td><img src="images/pc/icon_1'+(result.allAbilityList[i].rownum*1-1)+'.png"></td>'+
							// '<td>'+result.allAbilityList[i].rownum+'</td>'+
							'<td>'+result.allAbilityList[i].userId+'</td>'+
							'<td>'+result.allAbilityList[i].readValues+'</td>'+
							'<td>'+result.allAbilityList[i].borrowCount+'</td>'+
							'<td>'+result.allAbilityList[i].borrowTime+'</td>'+
							'<td><a href="javascript:woid(0)" class="btn btn-success save" data_id="'+result.allAbilityList[i].userId+'">详情</a></td>'+
							'</tr>');
					}else{
							$("#tbody_id").append('<tr class="gradeX">'+
							'<td>'+result.allAbilityList[i].rownum+'</td>'+
	...
	$(".save").on("click", function() {
		...
		// window.location.href = 'getMyReadAbility?jsonStr=' + url;
	});
...
// 模糊查询
	$("#book_search").click(function() {
		...
		Loading1.show();
		window.location.href='getStuReadAbilityRankAll?jsonStr='+json;
