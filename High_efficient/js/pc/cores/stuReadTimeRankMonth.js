...
$('.form_date').datetimepicker({
	...
	var userId=result.userId;
		...
		//1、循环输出列表
	if (result.readTimeMonthList != null && result.readTimeMonthList.length != 0)
	{
		$("#book_year").html(result.readTimeMonthList[0].year+"年"+result.readTimeMonthList[0].month+"学生阅读时间按月排名");
		for (var i = 0; i < result.readTimeMonthList.length; i++) 
		{
			if(result.pageNowTwo==1){
					if(result.readTimeMonthList[i].rownum <= 3){
						$("#tbody_id2").append('<tr class="gradeX">'+
						'<td><img src="images/pc/icon_1'+(result.readTimeMonthList[i].rownum*1-1)+'.png"></td>'+
						'<td>'+result.readTimeMonthList[i].userId+'</td>'+
						'<td>'+result.readTimeMonthList[i].borrowTime+'</td>'+						
							'</tr>');
					}else{
						$("#tbody_id2").append('<tr class="gradeX">'+
						'<td>'+result.readTimeMonthList[i].rownum+'</td>'+
						'<td>'+result.readTimeMonthList[i].userId+'</td>'+
						'<td>'+result.readTimeMonthList[i].borrowTime+'</td>'+
						'</tr>');
				}
...
// 模糊查询
	$("#book_search2").click(function() {
		...
		Loading1.show();
		window.location.href='getStuReadTimeRankMonth?jsonStr='+json;
