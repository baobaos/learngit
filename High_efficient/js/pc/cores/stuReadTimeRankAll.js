...
$('.form_date').datetimepicker({
	...
	var userId=result.userId;//1块查询条件
		...
		//1、循环输出列表
	if (result.stuReadTimeList != null && result.stuReadTimeList.length != 0)
	{
		for (var i = 0; i < result.stuReadTimeList.length; i++) 
		{
			if(result.pageNowOne==1){
					if(result.stuReadTimeList[i].rownum <= 3){
						$("#tbody_id").append('<tr class="gradeX">'+
						'<td><img src="images/pc/icon_1'+(result.stuReadTimeList[i].rownum*1-1)+'.png"></td>'+
						'<td>'+result.stuReadTimeList[i].userId+'</td>'+
						'<td>'+result.stuReadTimeList[i].borrowTime+'</td>'+
							'</tr>');
					}else{
						$("#tbody_id").append('<tr class="gradeX">'+
						'<td>'+result.stuReadTimeList[i].rownum+'</td>'+
						'<td>'+result.stuReadTimeList[i].userId+'</td>'+
						'<td>'+result.stuReadTimeList[i].borrowTime+'</td>'+
						'</tr>');
				}
...
// 模糊查询
	$("#book_search").click(function() {
		...
		window.location.href = 'getStuReadTimeRankAll?jsonStr='+json;
