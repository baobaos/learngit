	    ...
	    xAxis: [
	        {
	            ...
	            data: ["平均成绩", "平均消费", "阅读能力","上网时长","健康度","细心程度"],
	            position: "bottom",
	            axisLabel: {
	                textStyle: {
	                    color: "rgb(4, 241, 213)",
...
//动态生成数据
	...
	var json=ConsumptionRanking(result);					
		if(json==null)
		{
			...
			option.title.text=newData[0]+"年"+newData[1]+"月学生高薪就业群体分析";		
			...
			$(".Modular_color_02").next().text(result.highEmploymentStuCount+"人");
			$(".Modular_color_03").next().text((result.highEmploymentStuCount/result.sumStu*100).toFixed(2)+"%");
		            ...
		            success: function (result) {
		            	if(result.responseCode=="1"){
			            	...
			            	if(result.highOnlineAvgMoney!=null&&result.readEmploymentAvg!=null&&result.onlineLowAvg!=null
			            			...
			            			&&result.highAvgMoney!=null&&result.avgAllMoney!=null){
			            		var json=ConsumptionRanking(result);					
				        		if(json==null)
				        		{
				        			$map.html("<div class='nodata'>暂无数据.....</div>");
